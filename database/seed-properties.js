  ...
  for (const prop of properties) {
    const [result] = await connection.query(
      `INSERT INTO properties (title, slug, property_type, building_name, location, address,
        bedrooms, bathrooms, max_guests, parking_spots, size_sqft, price_per_night, short_description, description,
        map_url, plus_code, latitude, longitude, status, is_featured, meta_title, meta_description)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [prop.title, prop.slug, prop.property_type, prop.building_name, prop.location, prop.address,
        prop.bedrooms, prop.bathrooms, prop.max_guests, prop.parking_spots, prop.size_sqft, prop.price_per_night,
        prop.short_description, prop.description, prop.map_url, prop.plus_code, prop.latitude, prop.longitude,
